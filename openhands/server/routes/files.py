...
async def list_files(request: Request, conversation_id: str, path: str | None = None):
    ...
    To list files:
    ```sh
    curl http://localhost:3333/api/conversations/{conversation_id}/list-files
    ```
...
async def select_file(file: str, request: Request):
    ...
    To select a file:
    ```sh
    curl http://localhost:3333/api/conversations/{conversation_id}select-file?file=<file_path>
    ```
...
async def upload_file(request: Request, conversation_id: str, files: list[UploadFile]):
    ...
    To upload a files:
    ```sh
    curl -X POST -F "file=@<file_path1>" -F "file=@<file_path2>" http://localhost:3333/api/conversations/{conversation_id}/upload-files
    ```
